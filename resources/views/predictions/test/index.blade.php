...
@section('title', 'Prediksi Data Testing - SIDINI')
@section('page-title', 'Prediksi Data Testing CSV')
@section('page-subtitle', 'Upload file CSV tanpa label untuk diprediksi menggunakan model KNN')
...
<div class="loading-overlay" id="loadingOverlay">
    ...
    <p style="font-size: 1.1rem; font-weight: 600; margin: 0;">Memproses prediksi KNN data uji...</p>
...
<div class="medical-card fade-in">
    <h3 style="font-size:1.125rem; font-weight:700; color:var(--color-gray-800); margin:0 0 1.5rem;">
        📤 Upload File Data Uji
    </h3>

    <form action="{{ route('predictions.test.predict') }}" method="POST" enctype="multipart/form-data" id="uploadForm">
        @csrf

        <!-- Drop Zone -->
        <div class="upload-zone" id="dropZone" onclick="document.getElementById('test_dataset').click()">
            ...
            <p style="color:var(--color-gray-400); font-size:0.875rem; margin:0 0 1rem;">
                File data uji tanpa label (kolom <code>status</code> opsional)
            </p>
            ...
            <input type="file" name="test_dataset" id="test_dataset" accept=".csv" style="display:none;" onchange="handleFileSelect(this)">
        </div>
...
@include('partials.knn-evaluation-metrics', [
    'metrics' => $lastMetrics ?? [],
    'title' => '📊 Hasil Penilaian Terakhir (Data Testing)',
    'subtitle' => 'Confusion matrix & classification report dari prediksi data uji terakhir',
])
...
dropZone.addEventListener('drop', (e) => {
    ...
    if (dt.files.length) {
        const input = document.getElementById('test_dataset');
